...
- 층별 루트 제시 — 각 층에 들어갈 테마팩, 그 팩에서 챌 기프트, 조합을 수행할 시점, 시작 기프트와 관측 대상
- 불가능한 요구는 이유와 함께 알려줌 (그 층 범위에 팩이 없음 / 전용 기프트끼리 층이 충돌 / 덱이 조건을 못 채움)
- 계산 결과를 URL로 공유

## 쓰는 법

배포된 사이트를 열어 덱 → 기프트 → 루트 순서로 진행합니다. 로컬에서 돌리려면:
