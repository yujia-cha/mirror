...
- **고른 목표가 정리하는 기프트는 고를 수 없다**(`lib/entangle.ts`의 `blockedGifts`): 목표의 재료 트리에 있으면 **포함**(키워드가 달라도 잠근다 — 데스페라도 ⊃ 노이즈 섞인 무전기), 목표와 재료가 겹치는 다른 조합이면 **얽힘**이다. 상위를 고르면 이미 골라 둔 재료는 선택에서 빠진다. 타일은 `data-block`으로 이유를 남기고 제목이 상대 기프트를 말한다.
