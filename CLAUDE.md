...
- 우측 「전체 루트」 상단의 **「목표 기프트」 격자**는 `wanted` 전체(포기 포함)와 조합 목표의 재료를 추적기와 같은 `GiftTile` 토글로 보여 주고, 무대 팩 타일·추적기와 `run.giftStatus` 한 곳을 공유한다(누르면 획득 ↔ 해제, 실패는 층을 떠날 때의 자동 실패만). 타일은 **길게 누르기(1초)·우클릭·모서리 ⓘ**로 기프트 상세 시트를 연다(`useLongPress`, `PlanContext.openGift`가 시트를 한 번만 호스팅).
