...
- **게임 텍스트의 대괄호 버프 id는 빌드에서 한글로 바꾼다**: 원본은 「[Combustion] 횟수」처럼 id를 적고 게임이 실행 중에 이름을 덧칠한다. `scripts/lib/battle-keywords.ts`의 `localizeBuffTokens`가 `BattleKeywords.json`(언어별 id→이름)으로 양쪽 언어를 고쳐 쓴다. **이름을 모르는 id는 추측하지 않고 그대로 둔다**(인격 전용 버프 37개) — 빌드가 몇 개인지 알린다.
...
- `GiftIcon`의 **네 모서리**가 각각 하나씩 말한다: 좌상 티어, 우상 획득/실패, 좌하 반드시, **우하 키워드 배지**. 배지는 7개 상태 키워드가 색(`--color-kw-*`), 참격·관통·타격은 한 가지 잉크(`--color-kw-attack`)에 모양으로 가른다(마름모·원·사각). **범용은 배지를 그리지 않는다.** **조건 판정은 바깥 링**(`ring-ok`/`ring-bad`/회색)이다. 팔레트에서 색조를 쓰는 곳은 이 둘뿐이다. **`accent`는 색조가 없다** — 라이트에서 검정, 다크에서 흰색이고, 쓰는 자리는 **무대에 선 층 칸 한 곳**뿐이다. 지나간 것·확실성은 그대로 회색·채움·점선이 맡는다.
- **헤더에는 문 둘과 서랍 하나뿐이다**: 왼쪽 `≡ 덱`, 오른쪽 `≡ 전체 루트`(둘 다 `Menu` 아이콘 + 글자, 글자가 곧 접근 이름이다). 제목은 없다 — 패널 버튼이 어디로 가는지 말하므로 이름표가 따로 필요 없다. 초기화·공유·언어·화면 전환은 오른쪽 **「⋯」 팝오버**(`header-menu`)에 있다.
- 아이템 탭에서 **검색 중에는 결과가 검색창 바로 아래**로 온다(`searching`이면 격자를 위로, 아니면 탭 맨 아래). 필터·관측 슬롯·선택 트레이가 결과를 한 화면 아래로 밀지 않게 하기 위함이다.
- **선택 트레이는 `wanted`를 보는 방식일 뿐이다**: 정렬(선택 순서·키워드·이름)과 필터(키워드·팩)는 칩의 표시만 바꾸고 선택 자체는 건드리지 않는다. `wanted`는 **고른 순서대로** 쌓이고(id 정렬 아님) 공유 링크도 그 순서를 싣는다 — 플래너는 `expandRequirements`가 스스로 정렬하므로 루트는 순서에 영향받지 않는다. 트레이의 필터는 **선택한 것이 실제로 가진** 키워드·팩만 후보로 낸다.
...
- **우측 「전체 루트」는 팩의 흐름만 말한다**: 노선도 블록에는 팩 초상·이름만 두고 **기프트 아이콘을 두지 않으며**(무엇이 나오는지는 팩 시트), **조건 판정 카드도 없다**(판정은 아이템 타일의 링과 기프트 상세 시트가 말한다). 노선도 머리의 `start-cell`은 **두 줄**이다 — 위 `start-line`(시작 기프트 또는 「시작 {키워드}」), 아래 `observed-line`(관측 타일만, **「관측」 글자 없이**; 눈 배지가 이미 말하므로 그 말은 `aria-label`에만 둔다). 시작과 관측은 서로 다른 결정이라 한 줄에 섞지 않는다.
