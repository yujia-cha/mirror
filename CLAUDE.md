...
- `GiftIcon`의 **네 모서리**가 각각 하나씩 말한다: 좌상 티어, 우상 획득/실패, 좌하 반드시, **우하 키워드 배지**. 배지는 7개 상태 키워드가 색(`--color-kw-*`), 참격·관통·타격은 한 가지 잉크(`--color-kw-attack`)에 모양으로 가른다(마름모·원·사각). **범용은 배지를 그리지 않는다.** **조건 판정은 바깥 링**(`ring-ok`/`ring-bad`/회색)이다. 팔레트에서 색조를 쓰는 곳은 이 둘과 **`accent` 하나**뿐이다 — accent는 「지금 할 일」만 말한다(추천 팩 카드, 무대에 선 층 칸). 지나간 것·확실성은 그대로 회색·채움·점선이 맡는다.
- **무대는 행동 우선이다**: 추천 팩 하나만 카드(accent 테두리·「추천」 배지·accent 발 버튼)이고, 이 층에서 나가는 나머지 길(다른 루트 팩, 「넘기기」)은 그 옆의 줄이다. 당기기 제스처는 카드에도 줄에도 똑같이 붙는다.
