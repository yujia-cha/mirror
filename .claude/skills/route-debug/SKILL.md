...
UI에서 온 제보라면 공유 URL을 그대로 넘길 수 있다: `npm run route -- --share '#s=...'`. 공유 페이로드의 **반드시/보통(`priority`)·「재료도 목표」(`fusionGoal`)·출격 인원까지 그대로 재현**하므로 화면과 같은 숫자가 나와야 한다(다르면 그 자체가 단서다). 반대로 `--want`만 쓰면 `--must`를 주지 않는 한 **모든 기프트가 반드시**가 되어 앱 기본값과 다른 입력이 된다.
