...
3단계 스텝퍼를 버린다. 화면은 항상 **런 화면**이고 설정과 결과는 양쪽 패널에 있다. `store` v6: `step`·`run.active` 삭제, `ui { leftOpen, leftTab, rightOpen, rightTab }`(기기 저장), `run { currentFloor, stageFloor, visits, giftStatus }`.

- **셸**(`AppShell`): 헤더에 좌/우 패널 토글(`aria-expanded`·`aria-controls`) + 링크 복사·언어·테마. 데스크톱(≥1024px)은 `aside` 336px 두 개가 무대 옆에 붙고(`sticky`, 자체 스크롤, `ui`에 열림 저장), 폰은 드로어(`role=dialog`, 한 번에 한쪽, 로드 시 닫힘, Esc·배경·닫기). 패널 내용은 `@container`라 아이템·덱의 반응형은 패널 폭 기준이다.
- **좌측 탭**: 덱(`DeckStep`) / 아이템(`GiftsStep`, 빈 덱 카드의 「덱 탭으로」는 `onGoDeck`) / 루트 설정(`RouteSettings`: 시작 키워드, 관측 지정 목록(원하는 것 중 관측 가능한 것), 포함·포기 지정한 팩, 옵션 초기화, 「새 런」 confirm). 하단 내비·스텝퍼·요약 스트립은 없다.
- **무대**(`RunStage`, `data-mode`): `FloorHeader`(큰 층수 「4 / 15 · 1~5 · Hard」, 입장한 팩 배지 또는 「입장하지 않음 · 기록됨」, 「이전 층」·「입장하지 않음|다음 층」, 15칸 스트립 `floor-cell[data-state=entered|undecided|skipped|done]`·프런티어 뒤는 비활성) + 모드별 본문:
  - **undecided**(무대 = 프런티어): 「이 층에서 들어갈 수 있는 팩」 = 창이 이 층을 덮는 미지남 루트 팩(배정 층이면 「추천」 먼저, `stage-pack[data-recommended]`), 「입장하지 않음」 빈 카드(`skip-card`, 드래그 가능), **드롭 존**(`drop-zone[data-over]`, 「여기로 끌어다 놓으면 입장」), 「다른 팩 n」 `details`(이 층에 나올 수 있는 팩 전부, 팩·전용 기프트 이름 검색, 행마다 「입장」·팩 시트). 목표가 없으면 「아직 목표 기프트가 없습니다 · 아이템」 안내.
  - **entered**: `EnteredPack` — 팩 카드 96 + 「{n}층에 입장」 + 「입장 취소」(좌), 「이 팩에서만 나오는 기프트 n · 목표 m」 타일 그리드(우, 목표 먼저).
  - **skipped**(지난 층·방문 없음): 「지나간 층 · 다시 입장하면 기록이 바뀝니다」 + 루트 팩·다른 팩(빈 카드 없음).
  - **done**(`currentFloor` 16): 「15층까지 마쳤습니다 · 새 런」.
- **드래그**(`useDragEnter`): 카드 `pointerdown`(주 버튼, 버튼·링크·입력 위가 아닐 때) → 세로로 8px 넘게 움직이면 시작(가로 우선이면 스크롤) → `position: fixed` 고스트(`drag-ghost`, 「{팩} · 놓으면 입장」) + 원본 `opacity-40` → 존 사각형 안에서 놓으면 입장(빈 카드는 건너뜀). 탭은 드래그가 아니다. `aria-live`로 드래그 중 문구.
- **기프트 타일**(`GiftTile`): `<button aria-pressed>` 「{이름} 획득 표시」, `data-status=pending|got|failed`, `data-wanted`. 미획득 = 흑백 + 55%, 획득 = 컬러 + 체크 배지, 실패 = 45% + X 배지(누르면 획득), 목표 = `ring-1 ring-ink`. 팩 시트의 기프트 행도 런 컨텍스트에서는 타일이다(3단 Segmented 삭제).
- **settle**: 1층을 처음 떠날 때(첫 입장 또는 첫 건너뜀) 관측·시작 기프트를 `got`으로. 입장한 층을 「다음 층」으로 떠날 때 표시하지 않은 목표 전용 기프트를 `failed`로(`autoFailedFor`; 전용 = `exclusiveGifts` ∪ `clearRewardOf`). 「이전 층」으로 돌아가 타일을 누르면 `got`으로 바뀐다.
- **되돌리기**: `unvisitPack`으로 프런티어 바로 앞 층을 비우면 프런티어가 그 층으로 물러난다. 「이전 층」으로 방금 건너뛴 층에 돌아가도 같다. 더 오래된 층은 건너뜀으로 남는다.
- **우측 탭**: 전체 루트(`RoutePlanPanel`: 요약 「필요 팩·확보·포기 n·실패 n·미해결 n·근사·텍스트 복사」, 대안 탭, 세로 노선도(`MetroMap variant="vertical" detailMode="sheet"`, 지난 역·현재 역, 한 층 세그먼트는 「{n}층 방문 · 팩」 한 줄), 팩 충돌 카드(1열, 시트), 조건 판정) / 추적기(`Tracker`).
- **추적기**: 규칙 `tier ≥ 4 ∧ obtainable ∧ general ∧ 조건 없음 ∧ freelyAvailable` = 25종. 그룹 키워드 7 / 조각 7 / 기억 3 / 공격 유형 3 / 범용 5(`tracker-{group}` 카드, 「n/m」). 타일 32. `fusion.mixed`인 달의 기억을 획득으로 바꾸면 `fusion-notice`: 「합성으로 소모한 조각 2개·기억 3개는 미획득으로 표시하세요」 + 지금 획득인 재료마다 「미획득으로」 버튼. 달의 기억을 해제하면 닫힌다.
- **노선도·시트**: `MetroMap`에서 선택 모드·배너·역 버튼을 뺐다(`MetroRun = { currentFloor }`). `DetailSurface` 시트는 데스크톱에서도 보이고(패널 안에서 팝오버를 쓰지 않는다) 가운데 520px로 뜬다. `PackSheet`의 `EnterActions`: 입장했으면 「{n}층에 입장 · 입장 취소」, 무대 층에 나오는 팩이면 「입장 · {n}층」. 관측 지정 토글은 `currentFloor === 1`일 때만.
- **공유·저장**: 링크 v4 그대로(런 제외), 링크를 받으면 런 초기화. persist v6 migrate: `step` 삭제, `run.active === false`면 빈 런, `sanitizeRun`(currentFloor ≤ 16, stageFloor 클램프), `sanitizeUi`.
- 문구 키(M13 추가): `panelLeft/Right/Close`, `tabDeck/Gifts/RouteSettings/RoutePlan/Tracker`, `stageFloor`, `stageOf`, `stageEnterable`, `stageRecommended`, `stageNoRoutePack`, `stageNoGoals`, `stageHistoryHint`, `stageOtherPacks/Search/None`, `stageSkip`, `stageSkipped`, `stageSkipHint`, `stageEnter`, `stageEnterPack`, `stageDetail`, `stageDropZone`, `stageDropHint`, `stageDragging`, `stageUnenter`, `stageEntered`, `stageExclusives`, `stageExclusivesNone`, `stageGoalsFirst`, `stageGotHint`, `stageNext`, `stagePrev`, `stageDone`, `stageNewRun`, `stageNewRunConfirm`, `giftTileToggle`, `trackerTitle`, `trackerHint`, `trackerGroupKeyword/Shard/Memory/Attack/Plain`, `trackerFusionNotice`, `trackerFusionHeld`, `trackerUnmark`, `routeUnresolvedCount`, `settingsObserved`, `settingsObservedHint`, `settingsPreferred`, `settingsNone`. 삭제: `step1/2/3`, `stepsLabel`, `next`, `prev`, `toGifts`, `run*`(Start/End/EndConfirm/CurrentFloorHint/Hint/SetVisit/SetVisitHint/Selecting/SelectCancel/SelectFloor), `giftStatusLabel/Pending`, `aheadPacks/PacksHint/None/Search`.

## M14 추가 규칙 (당겨서 입장)

시안: https://claude.ai/code/artifact/25ce110b-c78a-490a-ae16-4cd851baaa4b. 드롭 존·고스트·「입장하지 않음」 빈 카드는 없어지고, 카드 자체를 당긴다(`usePullGesture`).

- **팩 카드**(`stage-pack`, 128px, 줄바꿈 행): 상태 배지(추천/루트 상태) → 팩 이미지 96×180 → 제목(버튼, 누르면 팩 시트; 시트는 카드 밖에서 렌더) → 전용 기프트 아이콘 20(목표 `ring-1 ring-ink`, 5개 초과 「+n」, `stage-pack-gifts`) → 하단 「입장 ⌄⌄」 버튼(`ChevronsDown`). 아래로 당기면 카드가 따라오고(`data-pulling`, `translateY`), 72px 넘으면 하단이 ink로 차며 「놓으면 입장」(`data-past=down`); 놓으면 입장, 못 미치면 180ms에 복귀. 위로는 0.3배 저항만. 쉴 때는 transform 없음.
- **다른 팩 입장**(`other-entry-card`): 점선 카드, `DoorOpen` 96×180, 「다른 팩 입장 · 루트 밖 팩으로 지나감」, 하단 「다음 층 ⌄⌄」. 당기거나 누르면 `nextFloor`(건너뜀). undecided일 때만.
- **카드 영역**(`pack-area` → 안쪽 `entered-pack`): 입장하면 `grid-template-rows 0fr→1fr` 200ms로 열린다. 위 핸들 「⌃⌃ 돌아가기 · 표시한 기프트를 초기화하고 팩 선택으로」(`area-back`, aria-label 「{팩} 돌아가기」) = `leave` → 방문 취소 + 그 팩 전용 기프트 상태 삭제 + 프런티어 후퇴; 본문 = 팩(좌, 이미지 96·이름·「n층에 입장」) + 전용 기프트 타일(우, 목표 먼저); 아래 핸들 「⌄⌄ 다음 층 · 표시하지 않은 목표는 실패로 기록」(`area-next`) = `next`. 영역 어디를 잡고 끌어도 방향으로 판정: 아래 72px 「놓으면 다음 층」, 위 72px 「놓으면 돌아가기 · 표시한 기프트 초기화」. 돌아간 뒤 200ms 동안 `pack-area-closing`이 접히며 사라진다.
...
- **시트는 포털**: `DetailSurface` sheet는 `document.body`로 `createPortal`한다. 패널이 `@container`라 그 안의 `position: fixed`가 패널 기준이 되기 때문이다(M14의 transform 문제와 같은 부류). 그래서 **시트는 그것을 연 표면이 아니라 `PlanProvider`가 호스팅한다** — 패널 안에 두면 모바일에서 패널이 닫힐 때 함께 언마운트된다.
- **모바일 패널은 페이지**(M24): 1024px 미만에서 좌·우 패널은 각각 `fixed inset-0`의 전체화면 페이지(`page-{side}`)로 `document.body`에 포털되고, 그동안 셸(`app-shell`)은 `inert`다. 다이얼로그가 아니므로 백드롭도 `aria-modal`도 없고 바깥 누름·Escape로 닫히지 않는다. 상단 바는 ←(`panelBack`) + 제목 `<h1>` + 탭 바이고, 앱 헤더는 페이지 뒤에 가린다 — 반대쪽 패널로는 페이지 **안에서** 간다(빈 루트의 「아이템」 버튼처럼). 세로 여백은 `env(safe-area-inset-*)`를 더한다.
...
## M29 추가 규칙 (무대의 글자 덜기 · 전체 루트는 팩만)

M28의 무대 「행동 우선」을 **되돌린 마일스톤이다.** 실제로 게임 옆에 띄워 놓고 쓰니 추천 팩만 큰
accent 카드로 세우는 것이 과했다 — 무대가 묻는 것은 「어느 팩에 들어갈까」 하나이고, 계획이 고른 팩은
`enterablePacks`의 정렬로 이미 맨 앞에 선다. 강조와 배지가 그 위에 얹혀 읽을 것만 늘렸다.

- **무대 팩 카드**: 루트 팩도 「넘기기」도 M28 이전의 같은 128px 카드로 돌아왔다(`StagePackCard` +
  `OtherEntryCard`, `StagePackRow`·`SkipRow` 삭제). 카드 테두리는 언제나 `border-line`이고 발 버튼의
  accent 채움도 없다. **accent는 무대에서 완전히 빠져** 층 스트립의 선 칸 한 곳에만 남는다.
- **배지 없음**: 카드 머리의 배지 슬롯을 통째로 지웠다 — 「추천」(`stageRecommended`, 키까지 삭제)과
  `PackStateBadge`(「포함 · N층」·「포함 지정」·「방문 · N층」…) 둘 다. `PackStateBadge` 컴포넌트는
  팩 시트·노선도·팩 충돌 카드가 계속 쓴다. 입장한 팩 영역의 「N층에 입장」(`stageEntered`) 배지도
  지웠다 — 층수는 무대 헤더가 말한다. 팩 시트의 같은 문장은 「입장 취소」 버튼을 설명하므로 남는다.
- **노선도는 팩만**: `packEntry`에서 기프트 아이콘을 뺐다. 블록은 팩 초상·이름·시트 앵커뿐이고, 세로
  배치의 블록 높이는 층 span에서 오므로 기하학은 그대로다.
- **조건 판정 카드 삭제**: 우측 패널이 판정을 다시 늘어놓지 않는다. 판정은 아이템 격자 타일의 링
  (`data-judgement`)과 기프트 상세 시트가 말하고, `shown.conditions` 자체는 그대로 계산된다.
- **시작과 관측은 두 줄**: `start-cell` 안을 `start-line`(시작 기프트 또는 「시작 {키워드}」)과
  `observed-line`(「관측」 라벨 + `ObservedTile`)로 나눴다. 런이 거저 받는 것과 별빛을 써서 지정한
  것은 다른 결정이라 한 줄에 섞지 않는다.

## M30 추가 규칙 (헤더·아이템 탭·흑백 강조색)

- **헤더**: 제목 없이 문 둘과 서랍 하나. 왼쪽 `≡ 덱`, 오른쪽 `≡ 전체 루트`(`Menu` 아이콘 + 글자,
  글자가 접근 이름이므로 `aria-label`을 달지 않는다), 그 왼쪽에 「⋯」(`moreActions`) 팝오버가 초기화·
  공유·언어·화면 전환 넷을 담는다(`header-menu`). 패널 버튼은 `aria-expanded`·`aria-controls`를
  그대로 들고 있어 `useDismiss`의 오프너 검사가 계속 통한다.
- **강조색**: `accent`는 색조를 버렸다 — 라이트 `#000000`(bg 위 19.44:1), 다크 `#ffffff`(18.71:1).
  쓰는 자리는 무대에 선 층 칸 하나뿐이고, 팔레트에서 색조는 키워드 배지와 판정 링만 남는다.
- **아이템 탭의 검색**: 질의가 있는 동안 결과 격자가 검색창 바로 아래로 올라온다. 필터·관측 슬롯·선택
  트레이가 그 사이에 있어 결과가 한 화면 아래에 있었다. 질의를 지우면 격자는 탭 맨 아래로 돌아간다.
- **선택 트레이**: 칩 줄 위에 정렬(`giftsSort`: 선택 순서·키워드·이름)과 필터(키워드·팩) 한 줄.
  `FilterSelect`를 그대로 쓰고, 「선택 순서」는 그 컴포넌트의 `all` 자리다. 정렬·필터는 표시만 바꾸고
  `wanted`는 건드리지 않는다(숨은 칩도 여전히 목표다). 필터 후보는 선택한 것이 실제로 가진 키워드와,
  선택한 것이 **전용으로 매인** 팩(`exclusiveTo` ∪ `clearRewardOf`)뿐이다.
- **`wanted`는 고른 순서**로 쌓인다(`toggleWanted`에서 id 정렬 제거). 「선택 순서」가 뜻을 가지려면
  그래야 하고, 공유 링크도 그 순서를 싣는다. 플래너는 `expandRequirements`가 goal을 정렬하므로
  루트는 순서에 영향받지 않는다.
- **노선도 관측 줄**: 「관측」 글자를 빼고 타일만 둔다. 눈 배지가 이미 말하므로 그 말은 줄의
  `aria-label`로만 남는다.
- **대괄호 버프 id**: 게임 텍스트의 `[Combustion]` 같은 id를 빌드가 `BattleKeywords.json`으로 고쳐
  쓴다(`scripts/lib/battle-keywords.ts`). 이름을 모르는 id는 그대로 둔다 — 추측한 이름보다 보이는
  id가 낫다.
